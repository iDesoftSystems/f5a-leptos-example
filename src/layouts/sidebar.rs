    ...
    view! {
        <div class="flex items-center px-2 py-4">
            <div class="flex items-center space-x-2">
                ...
                <span class="text-base font-bold text-blue-950">"F5A Leptos"</span>
            </div>
...
fn NavItem(children: Children, label: &'static str) -> impl IntoView {
    view! {
        <a class="flex items-center space-x-2 px-2 py-2 rounded-md hover:bg-slate-50">
...
fn SidebarOverlay(is_sidebar_open: RwSignal<bool>) -> impl IntoView {
    view! {
        <div
            class=move || if is_sidebar_open.get() {
...
fn aside_class_attrs(is_sidebar_open: RwSignal<bool>) -> impl Attribute {
    let base_classes = "fixed inset-y-0 left-0 z-50 w-72 flex flex-none flex-col transition-transform duration-300 transform bg-white border-r border-blue-100 px-2 py-1 lg:translate-x-0 lg:static lg:inset-0 shadow-xl lg:shadow-none";
